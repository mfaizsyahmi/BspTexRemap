...
def parse_dpg_elem(node,parent=None):
    ...
    if _n(node) == "START":
        ...
        # parse the first s_elem node, as there could be top level comments 
        if (elem := next((x.children[0] for x in node.children \
                          if _n(x.children[0])=="s_elem"),None)):
            return parse_dpg_elem(elem,parent)
