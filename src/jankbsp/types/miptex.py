...
class MipTex:
    ...
    def encode(self, dirty=False):
        ...
        if not self.is_external:
            ...
            body = b"\x00" * (MipTex.PALETTE_PAD + MipTex.PALETTE_SIZE)
            body[0:MipTex.PALETTE_PAD] = int(len(self.palette) // 3)\
                    .to_bytes(MipTex.PALETTE_PAD,byteorder='little')
            pal = self.palette[0:MipTex.PALETTE_SIZE] \
                    if len(self.palette) > MipTex.PALETTE_SIZE \
                    else self.palette
            body[MipTex.PALETTE_PAD:MipTex.PALETTE_PAD+len(pal)] = pal
    ...
    def fix_water(self, fog_color:Color|bytes|str=None,fog_intensity:int=None):
        ...
        if isinstance(fog_color, str):
            ...
            pal3 = fog_color[0:3]
        elif isinstance(fog_color,Color):
            pal3 = fog_color.encode()
        else:
            pal3 = self.palette[9:12] # copy existing value

        if fog_intensity:
            pal4 = bytes(fog_intensity) + b"\x00\x00"
        else:
