    ...
    with dpg.window(tag="Primary Window",no_scrollbar=True):
        ...
        with dpg.menu_bar() as menubar:
            ...
            with dpg.menu(label="Textures"):
                ...
                _bind(BindingType.Value, _propbind(app.data,"auto_load_wads"))
        ...
        with dpg.table(resizable=True,height=-8) as mainLayoutTable: # header_row=False,
            ...
            app.view.bind(col1, BindingType.FormatLabel, _propbind(app.data,"mat_set"),
                          ["Materials {}",
                           lambda mat: f"({len(+mat)}/{len(mat)})" \
                           if app.data.matpath else ""])

            col2 = dpg.add_table_column(label="Textures",init_width_or_weight=3)
            app.view.bind(col2, BindingType.FormatLabel, _propbind(app,"view"),
                          ["Textures {}",
                           lambda view: "({}M + {}X = {}T, {}V)"\
            ...
            with dpg.table_row() as mainLayoutRow:
                ...
                with dpg.child_window(border=False):
                    ...
                    _bind(BindingType.Value,_propbind(app.data,"matpath"))
                    ...
                    with dpg.collapsing_header(label="Summary",default_open=True):
                        ...
                        _bind(BindingType.MaterialSummaryTable)
                    ...
                    with dpg.collapsing_header(label="Entries"):
                        ...
                        _bind(BindingType.Value,_propbind(app.view,"filter_matchars"))
                        
                        ## material name filter
                        dpg.add_input_text(label="filter name",hint=f"Material name")
                        _bind(BindingType.Value,_propbind(app.view,"filter_matnames"))
                        
                        ## Material entries table
                        ## Mat | Name | Usable
                        dpg.add_table(resizable=True,sortable=True,callback=_sort_table)
                        _bind(BindingType.MaterialEntriesTable)
                ...
                ### textures pane ###
                with dpg.child_window(autosize_y=False,menubar=True,tag="TexWin",
                    ...
                    with dpg.menu_bar():
                        ...
                        with dpg.menu(label="Show:All") as mnuTexShow:
                            ...
                            _bind(BindingType.TextMappedValue,
                                  _propbind(app.view,"gallery_show_val"),
                            ...
                            _bind(BindingType.FormatValue, _propbind(app.view,"wadstats"),
                                  ["Referenced WAD files: {}",lambda stats:len(stats) ] )

                            dpg.add_group(tag="grpWadlist")
                            _bind(BindingType.WadListGroup)
                        ...
                        with dpg.menu(label="Size:Stuff") as mnuTexSize:
                            ...
                            for i, text in enumerate(mappings.gallery_sizes):
                                ...
                                _bind(BindingType.ValueIs,
                                      _propbind(app.view,"gallery_size_val"),
                                      i )

                            ## [----|-----] scale slider
                            dpg.add_slider_float(label="scale",max_value=16.0,
                                                 clamped=True)
                            _bind(BindingType.Value,
                        ...
                        with dpg.menu(label="Sort:Sure"):
                            _bind(BindingType.FormatLabel,
                                  _propbind(app.view,"gallery_sort_val"),
                                  ["Sort:{}",
                        ...
                        with dpg.menu(label="Filter:OFF") as mnuFilter:
                            _bind(BindingType.FormatLabel,_propbind(app,"view"),
                                  ["Filter:{}",
                            ...
                            _bind(BindingType.Value,_propbind(app.view,"filter_str"))

                            ## [ ] Textures without materials only
                            dpg.add_checkbox(label="Textures without materials only")
                            _bind(BindingType.Value,_propbind(app.view,"filter_unassigned"))

                            ## [ ] Exclude radiosity textures
                            dpg.add_checkbox(label="Exclude radiosity textures")
                            _bind(BindingType.Value,_propbind(app.view,"filter_radiosity"))
                ...
                with dpg.child_window(border=False):
                    ...
                    _bind(BindingType.Value, _propbind(app.data,"auto_parse_ents"))
                    _help("TODO")
